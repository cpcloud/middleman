...
- `Close()` -- wraps the actual work in `closeOnce.Do`: closes `hub.done` (all SSE handlers immediately observe this in their select loops and exit, bypassing any buffered events), then under `mu` calls `unsubscribe(id)` for every remaining subscriber to clean up channels. Called by `Server.Shutdown` before `httpSrv.Shutdown`. Because handlers select on `<-done` with the same priority as `<-ch`, the `done` close fires immediately regardless of buffered channel state — no 80s drain delay. This ensures all SSE handlers exit within one `writeDeadline` (5s, if one is mid-write) before `httpSrv.Shutdown`'s deadline starts, well within `Run`'s 5s shutdown timeout.

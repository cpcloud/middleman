...
**Stale-response guard (applies to all stores below):** Every store that loads data via async fetches (pulls, issues, activity, detail) maintains a module-level monotonic `requestVersion` counter (a simple `let requestVersion = 0`). Each load function increments the counter at call time, captures the value, and checks it after `await`: if the captured version no longer equals the current `requestVersion`, the response is stale (a newer request was issued while this one was in flight) and the result is discarded silently. This prevents a slow in-flight poll or mount-triggered fetch from overwriting data returned by a newer SSE-triggered refresh. The counter is per-store, not global, because stores are updated independently. No `AbortController` is needed — the redundant fetch completes and is simply ignored on arrival.

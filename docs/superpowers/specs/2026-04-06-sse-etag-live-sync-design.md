...
- Stale poll vs SSE update: start a `refreshSyncStatus()` poll (mock delayed), then call `updateSyncFromSSE({ running: true, ... })`, then resolve the delayed poll with idle state. Assert stale response is discarded. Same pattern as above but via SSE path instead of optimistic trigger.

...
Methods:
- `Subscribe(ctx context.Context) <-chan Event` -- under `mu`, creates a buffered channel (buffer size 16), pre-loads `lastSyncStatus` into the channel if non-nil, registers the channel under a fresh `id`, and releases the lock. Spawns a goroutine that calls the shared `unsubscribe(id)` helper when ctx is canceled. Returns the channel. Note: only `sync_status` is cached; `data_changed` is not. Reconnect catch-up for missed `data_changed` events is handled client-side (the frontend fires a full refresh on every SSE `open` event — see Frontend SSE Client).
...
- `Close()` -- under `mu`, calls `unsubscribe(id)` for every remaining subscriber, clearing the map. Called by `Server.Shutdown` before `httpSrv.Shutdown` to ensure all SSE handlers exit their select loops (by observing `ok == false` on the closed channel) before the HTTP server's graceful shutdown deadline begins. Without this, active SSE connections would keep handlers alive indefinitely, causing `httpSrv.Shutdown` to block until its context deadline and then error.
...
**`sync.svelte.ts`:**
- All sync-state mutations go through a single internal `applySyncState(status: SyncStatus)` helper that first increments `syncVersion`, then updates `syncState`, runs `adjustPollingSpeed`, and fires `onSyncComplete` on running-to-idle transitions. This ensures every write path — `refreshSyncStatus()` (after `await`, guarded by version check), `updateSyncFromSSE()`, and `triggerSync()`'s optimistic `{ running: true }` — bumps the version counter, so stale in-flight HTTP responses from any source are correctly discarded.
- New `updateSyncFromSSE(status: SyncStatus)` function: calls `applySyncState(status)`. Same as `refreshSyncStatus` but without the HTTP call.
...
- `Run` shutdown ordering — sync is in-flight when bind fails: install a mock GitHub client whose first list call blocks on a test channel (`runOnceEnteredCh`) until released. **The test waits on `runOnceEnteredCh` before pre-binding the target port**, ensuring the syncer goroutine has actually entered `RunOnce` before the bind error path is triggered. Sequence: (1) call `Run` in a goroutine, (2) wait for `runOnceEnteredCh` (proves `RunOnce` started), (3) pre-bind the target port with a sentinel listener so `Run`'s `Server.Listen(addr)` will fail with "address already in use". This requires `Listen` to be called lazily or the port to be pre-bindable by the test after `Run` begins — the simplest approach is to have the mock's blocking point fire before `Listen` is reached (which is true because `Syncer.Start` is called before `Server.Listen` in `Run`). Assert: (1) `Run` returns the wrapped bind error, (2) `runOnceEnteredCh` was reached (guaranteed by the wait), (3) by the time `Run` returns, the syncer goroutine has fully exited (`Stop()` already returned and a follow-up `Status()` shows `Running: false`), and (4) the DB handle is closed AFTER the syncer goroutine exited, not before. The release of the mock channel happens automatically once `cancelSync()` is called inside `Run`'s deferred chain — the mock detects the canceled context and unblocks. This is the regression guard for the shutdown race where `defer app.DB.Close()` would otherwise run while a `RunOnce` was still mid-flight after a bind error.

...
- `Close()` -- wraps the actual work in `closeOnce.Do`: closes `hub.done` (SSE handlers observe this in their two-level select and exit after at most one additional buffered event write), then under `mu` calls `unsubscribe(id)` for every remaining subscriber to clean up channels. Called by `Server.Shutdown` before `httpSrv.Shutdown`. Handlers exit within at most `2 * writeDeadline` (10s) of the close — see shutdown bound analysis in SSE handler step 5. `Run`'s shutdown timeout is set to 15s to provide margin.
